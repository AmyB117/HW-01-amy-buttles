...
There are **13245** rows and each row represents a specific topic regarding the airbnb. e.g. how many rooms, reviews, etc.
...
````{r}
ggplot(data = bedroom, mapping = aes(x = beds) +
  geom_histogram(binwidth = price) +
  facet_wrap(~3
  )
````
...
ggplot( aes(x = airbnb, y = listing_url, fill = price)) +
geom_density_ridges()
